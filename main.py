...
class Framer:
    ...
    def generate(self):
        ...
        mean_colors = cv2.resize(mean_colors, (self.__x, self.__y))

        if cv2.imwrite(self.__output, mean_colors):
            print("Saved to {}".format(self.__output))
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("size", help="Dimensions of the output file, {x}x{y}. If the x value is 0, it will "
                                     "automatically be 1 pixel per frame.")
